...
type ChartPatternTone = 'positive' | 'negative' | 'neutral';
...
const toneLabels: Record<ChartPatternTone, string> = {
  positive: '긍정 관찰',
  negative: '주의 관찰',
  neutral: '확인 필요',
};
...
const chartPatterns: ChartPattern[] = [
  {
    ...
    category: 'reversal',
    tone: 'negative',
    toneSummary: '고점 돌파가 막히고 기준선 아래로 밀리면 상승 힘 약화를 의심해볼 수 있습니다.',
  ...
  },
  {
    ...
    category: 'reversal',
    tone: 'positive',
    toneSummary: '저점 방어 후 기준선 위로 회복하면 하락 압력 완화를 관찰할 수 있습니다.',
  ...
  },
  {
    ...
    category: 'reversal',
    tone: 'negative',
    toneSummary: '오른쪽 고점이 낮아지고 목선을 이탈하면 기존 상승 리듬 약화를 살펴봅니다.',
  ...
  },
  {
    ...
    category: 'reversal',
    tone: 'positive',
    toneSummary: '오른쪽 저점이 높아지고 목선을 회복하면 하락 리듬 변화 여부를 봅니다.',
  ...
  },
  {
    ...
    tone: 'positive',
    toneSummary: '높아지는 저점이 유지되고 상단을 회복하면 긍정 흐름으로 관찰할 수 있습니다.',
  ...
  },
  {
    ...
    category: 'compression',
    tone: 'negative',
    toneSummary: '낮아지는 고점이 이어지고 하단을 이탈하면 주의 흐름으로 봅니다.',
  ...
  },
  {
    ...
    tone: 'positive',
    toneSummary: '상승 후 박스권 상단을 회복하면 흐름 지속 가능성을 학습용으로 확인합니다.',
  ...
  },
  {
    ...
    category: 'continuation',
    tone: 'negative',
    toneSummary: '하락 후 박스권 하단을 이탈하면 약한 흐름 지속 여부를 관찰합니다.',
  ...
  },
  {
    ...
    category: 'compression',
    tone: 'negative',
    toneSummary: '상승 중 폭이 좁아진 뒤 하단선을 이탈하면 과열 둔화 가능성을 봅니다.',
  ...
  },
  {
    ...
    category: 'compression',
    tone: 'positive',
    toneSummary: '하락 중 폭이 좁아진 뒤 상단선을 회복하면 압력 완화 여부를 봅니다.',
  ...
  },
  {
    ...
    category: 'channel',
    tone: 'negative',
    toneSummary: '우상향 채널 하단을 벗어나면 기존 상승 리듬이 깨졌는지 확인합니다.',
  ...
  },
  {
    ...
    category: 'channel',
    tone: 'positive',
    toneSummary: '우하향 채널 상단을 회복하면 하락 리듬 변화 가능성을 관찰합니다.',
...
export function ChartPatternsPage() {
  ...
  return (
    <section className="content-section pattern-page">
      ...
      <section className="pattern-tone-legend" aria-label="패턴 방향성 안내">
        <div>
          <span className="card-label">FLOW SIGNAL</span>
          <h2>색상으로 먼저 흐름을 구분하세요</h2>
          <p>긍정/주의 표시는 학습용 관찰 방향입니다. 실제 판단은 기준선 이탈·회복, 종가, 거래량을 함께 확인해야 합니다.</p>
        </div>
        <div className="pattern-tone-legend-items">
          <span className="pattern-tone-badge pattern-tone-positive">긍정 관찰</span>
          <span className="pattern-tone-badge pattern-tone-negative">주의 관찰</span>
          <span className="pattern-tone-badge pattern-tone-neutral">확인 필요</span>
        </div>
      </section>
      ...
      <div className="pattern-grid">
        {filteredPatterns.map((pattern) => (
          <article className={`pattern-card pattern-card-${pattern.tone}`} key={pattern.title}>
            ...
            <div className="pattern-card-body">
              <div className="pattern-card-badges">
                ...
                <span className={`pattern-tone-badge pattern-tone-${pattern.tone}`}>{toneLabels[pattern.tone]}</span>
              </div>
              <h2>{pattern.title}</h2>
              <p className="pattern-tone-summary">{pattern.toneSummary}</p>
              <p>{pattern.summary}</p>
